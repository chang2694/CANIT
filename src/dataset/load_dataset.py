...
def generate_dataset(file_name, file_path, org_columns, chunk_size=10000):
    ...
    #
    # for chunk in pd.read_csv(file_path, skiprows=1, names=org_columns, chunk_size=chunk_size):
    #     chunk['ID'].replace("id", "", regex=True, inplace=True)
    ...
    for chunk in pd.read_csv(file_path, skiprows=1, names=org_columns, chunksize=chunk_size):
        chunk['ID'].replace("id", "", regex=True, inplace=True)
        chunk.reset_index(drop=True, inplace=True)
        df_extended_list.append(chunk[['ID', 'Label', 'Time']])

        for target_id in chunk['ID'].unique():
            df_perID = chunk[chunk['ID'] == target_id].T.dropna().T
            # Appending ID number at the end of the signal column
            column_rename_dict = {}
            for org_column_name in set(df_perID.columns) - set(['ID','Label','Time']):
                new_column_name = org_column_name.replace("Signal", "Sig_")
            ...
            df_perID = df_perID.rename(column_rename_dict, axis=1)
            new_column_names = list(column_rename_dict.values())
            df_extended_list.append(df_perID[new_column_names])

            #merging ID wise data frame in a singel combined one
            df_extended = pd.concat(df_extended_list, axis = 1, ignore_index=True)
